...
class EntityAttentionMAICAgent(nn.Module):
    def __init__(self, input_shape, args):
        super(EntityAttentionMAICAgent, self).__init__()
        self.args = args
        ...
        self.inference_net = nn.Linear(args.attn_embed_dim + args.n_actions, args.msg_dim * 2)
        self.zt = None
    ...
    def forward(self, inputs, hidden_state, ret_attn_logits=None, new_msg=False):
        ...
        hs = th.stack(hs, dim=1)  # Concat over time #bs,ts,na,hd
        ...
        #####
        selected_action = th.max(q, dim=-1)[1].unsqueeze(-1)#bs, ts, self.args.n_agents,1
        one_hot_a = th.zeros(bs,ts, self.args.n_agents, self.args.n_actions).to(self.args.device).scatter(-1, selected_action, 1).view(bs*ts,self.args.n_agents,-1)
        msg_q_logits = self.inference_net(th.cat([x2_coach, one_hot_a], dim=-1)).reshape(bs,ts,self.args.n_agents, self.args.msg_dim*2)
        #####
...
class ImagineEntityAttentionMAICAgent(EntityAttentionMAICAgent):
    def __init__(self, *args, **kwargs):
        super(ImagineEntityAttentionMAICAgent, self).__init__(*args, **kwargs)
    ...
    def forward(self, inputs, hidden_state, imagine=False, new_msg=False, **kwargs):
        if not imagine:
            return super(ImagineEntityAttentionMAICAgent, self).forward(inputs, hidden_state, new_msg=new_msg)
        entities, obs_mask, entity_mask = inputs
        bs, ts, ne, ed = entities.shape
        ...
        q, h, zt_logits, msg_q_logits = super(ImagineEntityAttentionMAICAgent, self).forward(inputs, hidden_state, new_msg=new_msg)
