...
class EntityAttentionIMACAgent(nn.Module):
    def __init__(self, input_shape, args):
        super(EntityAttentionIMACAgent, self).__init__()
        self.args = args
    ...
    def forward(self, inputs, hidden_state, ret_attn_logits=None, new_msg=False):
        ...
        msg_q_logits = th.cat([th.zeros(bs,ts,self.args.n_agents, self.args.msg_dim),self.args.logvar * th.ones(bs,ts,self.args.n_agents, self.args.msg_dim)],dim=-1).float().to(self.args.device)
...
class ImagineEntityAttentionIMACAgent(EntityAttentionIMACAgent):
    def __init__(self, *args, **kwargs):
        super(ImagineEntityAttentionIMACAgent, self).__init__(*args, **kwargs)
    ...
    def forward(self, inputs, hidden_state, imagine=False, new_msg=False, **kwargs):
        if not imagine:
            return super(ImagineEntityAttentionIMACAgent, self).forward(inputs, hidden_state, new_msg=new_msg)
        entities, obs_mask, entity_mask = inputs
        bs, ts, ne, ed = entities.shape
        ...
        q, h, zt_logits, msg_q_logits = super(ImagineEntityAttentionIMACAgent, self).forward(inputs, hidden_state, new_msg=new_msg)
